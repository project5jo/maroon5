...
$inputAccount.addEventListener("keyup", (e) => {
  setTimeout(() => {
    ...
    if (e.target.value === "" || e.target.value.length === 0) {
      ...
      const URL = `/checkid?account=${encodedAccount}`;
...
$inputFirstPassword.addEventListener("input", (e) => {
  setTimeout(() => {
    ...
    if (e.target.value.trim() === "" || e.target.value.length === 0) {
      ...
      if (
        $inputSecondPassword.value.trim() !== "" ||
...
function checkAllFlagArray() {
    ...
    for (let key in flagArray) {
      console.log('flag', flagArray[key]);
      if (!flagArray[key]) {
        $check.style.backgroundColor = 'rgba(82, 51, 51, 0.3)';
        $check.disabled = true; // 버튼 비활성화
        return false;
      }
    }
    $check.style.backgroundColor = '#4d3333';
    $check.disabled = false; // 버튼 활성화
    return true; // 모든 값이 true면 true 반환
}
